...
export async function checkDomainRelevance(bot, userMessage, history = []) {
  ...
  if (history.length > 0 && hasFollowUpSignal && !foreignHits.length) {
    return {
  ...
  // A tiny ellipsis question ("price?", "mileage?", "which is best?") is a real
  // continuation when the conversation was JUST about this field's own words.
  const histOwn = hits(historyText || '', own);
  const carryOver = wordCount <= 6 && histOwn.length >= 1;
  if (isSelfContained && !carryOver) {
